# 💾 Batch 2 — Servisler ve Veritabanı Katmanı

> IndexedDB, POS/Ödeme, Dışa Aktarım, Arşiv ve Performans Sistemleri
...
| Servis | İşlem | Hedef | Mevcut | Durum |
|--------|-------|-------|--------|-------|
| **ProductDB** | Barkod Sorgulama | ≤ 40ms | ~30ms | ✅ |
| **ProductDB** | Stok Güncelleme | ≤ 60ms | ~45ms | ✅ |
| **ProductDB** | 1000 ürün BulkInsert | ≤ 3s | ~2.5s | ✅ |
| **SalesDB** | 30 Günlük Sorgu | ≤ 80ms | ~65ms | ✅ |
| **CreditServices** | 100 Borç Ödemesi | ≤ 200ms | ~150ms | ✅ |
| **CashRegisterDB** | Satış Kaydı | ≤ 80ms | ~60ms | ✅ |
| **ReceiptService** | PDF Oluşturma | ≤ 350ms | ~280ms | ✅ |
| **ExportServices** | Excel Dışa Aktarım | ≤ 2s | ~1.5s | ✅ |
| **POSServices** | Cihaz Bağlantısı | ≤ 4s | ~3.5s | ✅ |
...
Bu doküman, RoxoePOS'un servis ve veri katmanını detaylandırır. IndexedDB tabanlı depolama, POS/ödeme servisleri, dışa aktarım sistemleri ve performans optimizasyonları hakkında kapsamlı bilgi sunar.
...
- `productDB.ts` - Ürün, kategori ve grup yönetimi
- `salesDB.ts` - Satış kayıtları ve raporlama
- `creditServices.ts` - Veresiye ve müşteri yönetimi
- `cashRegisterDB.ts` - Kasa dönemleri ve nakit akışı
- `UnifiedDBInitializer.ts` - Merkezi DB başlatıcı
- `dbService.ts` - Veritabanı yardımcı fonksiyonları
- `encryptionService.ts` - Veri şifreleme servisi

### 💳 POS ve Fiş Sistemleri
- `posServices.ts` - POS cihaz entegrasyonu
- `receiptService.ts` - PDF fiş oluşturma

### 📤 İçe/Dışa Aktarım
- `exportServices.ts` - Excel raporlama
- `importExportServices.ts` - Ürün içe/dışa aktarma

### ⚙️ Optimizasyon ve Performans
- `ArchiveService.ts` - Veri arşivleme
- `SmartArchiveManager.ts` - Akıllı arşiv yönetimi
- `IndexOptimizer.ts` - İndeks optimizasyonu
- `AIIndexAnalyzer.ts` - AI destekli indeks analizi
- `PerformanceMonitor.ts` - Performans izleme
- `CloudSyncManager.ts` - Bulut senkronizasyon

### 📄 Tip Tanımları
- `product.ts`, `sales.ts`, `credit.ts`
- `pos.ts`, `receipt.ts`, `cashRegister.ts`

> 📝 **Not:** UI/Hook bileşenleri [Batch 3](components-batch-3.md)'te, POS/Modal detayları [Batch 5](components-batch-5.md)'te yer alır.

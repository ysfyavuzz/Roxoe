    ...
    try {
      ...
      if (idxNames && (needStatus || needPayment || needDate)) {
        ...
        if (needStatus && idxNames.contains('status')) {
          const byStatus = await getAllFromIndex('status', filter.status as IDBValidKey)
          candidate = byStatus
        }
        if (needPayment && idxNames.contains('paymentMethod')) {
          const byPayment = await getAllFromIndex('paymentMethod', filter.paymentMethod as IDBValidKey)
